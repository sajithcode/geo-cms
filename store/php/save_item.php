...
try {
    ...
    if (!empty($category_id)) {
        $stmt = $pdo->prepare("SELECT id FROM store_categories WHERE id = ?");
    ...
    try {
        ...
        if (!empty($item_id)) {
            ...
            $stmt = $pdo->prepare("SELECT image_path FROM store_items WHERE id = ?");
        ...
        if (!empty($item_id)) {
            ...
            if ($image_path && strpos($image_path, 'item_new_') !== false) {
                ...
                $new_path = '../../uploads/store/' . $new_filename;
                $new_image_path = 'uploads/store/' . $new_filename;
                
                if (rename($old_path, $new_path)) {
                    $image_path = $new_image_path;
                    // Update database with correct path
                    $stmt = $pdo->prepare("UPDATE store_items SET image_path = ? WHERE id = ?");
